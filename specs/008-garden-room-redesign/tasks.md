...
- [x] T017 [P] Add section `id` attributes to each section wrapper in `apps/web/src/routes/GardenRoom.tsx` for anchor link navigation: `id="garden_room_hero"`, `id="product-range"`, `id="why-steel-frame"`, `id="planning-permission"`, `id="garden-room-gallery"`, `id="testimonials"`, `id="faq"`, `id="contact-cta"`. Verify the hero "Explore Our Ranges" link scrolls smoothly to `#product-range`.

- [x] T018 [P] Run `pnpm --filter @modular-house/web optimise:images` to ensure all images in `/public/resource/garden-room/` have WebP and AVIF variants. Verify that every image referenced in `garden-room-data.ts` has matching `.webp` and `.avif` files.

- [x] T019 Perform responsive testing of `apps/web/src/routes/GardenRoom.tsx`. The page now has a 9-section layout: (1) Hero, (2) Why Steel Frame (FeatureSection), (3) Product Showcase (ProductShowcase — 50/50 split), (4) Infinite Gallery (InfiniteMasonryGallery), (5) Product Range (ProductRangeGrid with InfoBanner for planning permission), (6) Construction Comparison (ComparisonSection), (7) Testimonials, (8) FAQ, (9) CTA. Check at 3 breakpoints: (1) Desktop 1440px — product range cards in 4 columns, features in 4 columns, ProductShowcase 50/50 split side-by-side, ComparisonSection table legible with all three columns, InfiniteMasonryGallery scrolls horizontally, InfoBanner renders inline within ProductRangeGrid header. (2) Tablet 768px — product range cards in 2 columns, features stack or 2 columns, ProductShowcase stacks vertically, ComparisonSection remains usable (may switch to card view), InfoBanner stacks content. (3) Mobile 375px — all content single column, no horizontal overflow, text readable, CTAs full-width, InfiniteMasonryGallery touch-scrollable, ComparisonSection card/deep-dive view usable, QuickViewModal fits viewport. Fix any layout issues found. Also verify at 320px (narrow) and 2560px (ultra-wide, content should be centred in max-width container).

- [x] T020 Perform accessibility review of the page (9-section layout). Check: (1) Heading hierarchy — one `<h1>` in hero, each section has `<h2>`, product cards use `<h3>`. (2) All images have descriptive `alt` text (including InfiniteMasonryGallery images and ProductShowcase product images). (3) All CTA buttons/links are keyboard-focusable with visible focus indicators (including ProductShowcase rows, ComparisonSection toggle buttons, and QuickViewModal close/CTA buttons). (4) Product card badges have sufficient colour contrast. (5) FAQ accordion items have correct `aria-expanded` and `aria-controls` attributes; expand/collapse works via keyboard (Enter/Space). (6) InfiniteMasonryGallery lightbox is keyboard-navigable (Escape to close, arrow keys); drag/scroll interaction does not trap keyboard focus. (7) QuickViewModal: focus is trapped within the modal when open, Escape key closes it, focus returns to the trigger element on close. (8) ComparisonSection: "Side by Side" / "Deep Dive" toggle is keyboard-accessible; table cells have appropriate headers or aria labels for screen readers. (9) InfoBanner within ProductRangeGrid has correct `aria-label` attribute and status items are announced meaningfully. (10) Page is navigable via Tab key from top to bottom without getting trapped.
