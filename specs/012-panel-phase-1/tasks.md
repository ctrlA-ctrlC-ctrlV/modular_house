...
- [x] T086 Implement the Login page
      ...
      > note: Login page — email+password form w/ react-hook-form + Zod, two-column login v1 layout (branded left panel bg-primary lg:w-1/3 + form on right), inline SVG CommandIcon, "Forgot password" link, no Google (FR-005), no registration (FR-006). Tests: 10 assertions (email, password, submit, no-Google, forgot link, form). Files: Login.tsx, preAuth.test.tsx.
      > reviewed: PASS — Tests 171/171 (later 180/180). FR-005/FR-006 met. Initial impl was single-column; fixed in fed5c56 to match reference template split-screen. Input forwardRef added (fixes RHF ref warning). Duplicate h1 (cosmetic, non-blocking).
...
- [x] T087 Implement the TwoFactor page
      ...
      Done when: relevant T085 assertions pass.
      Refs: T-F6, B9, FR-031
      > note: TwoFactor page — 6-digit InputOTP (3-3 separator), resend button (disabled state for cooldown), challengeId prop (B9), verify submit. Two-column login v1 shell matching Login.tsx. Tests: 4 assertions (OTP slots, verify, resend, back link). Files: TwoFactor.tsx, preAuth.test.tsx.
      > reviewed: PASS — Tests 180/180, 19/19 preAuth. onSubmit `{challengeId, code}` matches OpenAPI VerifyTwoFactorRequest. FR-031 AT association correct via sr-only FieldLabel. Resend countdown deferred to T116. Nits: test+impl same commit (precedented); duplicate h1 (cosmetic, same as T086).
...
- [x] T088 Implement the ForgotPassword page
      ...
      > note: ForgotPassword page — email entry with Zod validation, "Send reset link" button, neutral confirmation (C4, no account-existence disclosure) with "try again" control, "Back to login" link. Two-column login v1 shell matching Login.tsx. Tests: 4 assertions (email, submit, back link, form). Files: ForgotPassword.tsx, preAuth.test.tsx.
      > reviewed: PASS — Tests 180/180, 19/19 preAuth. FR-014 email entry met. FR-015 neutral confirmation shown unconditionally (C4). onSubmit `{email}` matches OpenAPI ForgotPasswordRequest. No scope creep. Nits: test+impl same commit (precedented); duplicate h1 (cosmetic).
